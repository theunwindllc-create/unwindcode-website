...
test('transmissions archive is a bilingual proof library with reader lanes', async () => {
  ...
  assert.equal(source.match(/<a href="\/transmissions\/[^"]+" class="tx-card"/g)?.length ?? 0, 36, 'transmissions archive should preserve 36 transmission pages');
  ...
  for (const snippet of [
    ...
    '35 dispatches as a proof library',
